...
void AnimationClip::play()
{
    ...
    _timeStarted = Toolkit::cur()->getGameTime();
}
...
bool AnimationClip::update(float elapsedTime)
{
    ...
    if (isClipStateBitSet(CLIP_IS_FADING_OUT_BIT))
    {
        ...
        {
            GP_ASSERT(_crossFadeToClip);
            _crossFadeOutElapsed = (Toolkit::cur()->getGameTime() - _crossFadeToClip->_timeStarted) * fabs(_speed); 
...
void AnimationClip::onBegin()
{
    ...
    if (_speed >= 0)
    {
        _elapsedTime = (Toolkit::cur()->getGameTime() - _timeStarted) * _speed;
    ...
    {
        _elapsedTime = _activeDuration + (Toolkit::cur()->getGameTime() - _timeStarted) * _speed;
