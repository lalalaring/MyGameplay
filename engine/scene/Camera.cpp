...
Camera* Camera::create(Properties* properties)
{
    ...
    else
    {
        ...
        aspectRatio = (float)Toolkit::cur()->getViewport().width / Toolkit::cur()->getViewport().height;
    }
    ...
    case Camera::ORTHOGRAPHIC:
        ...
        camera = createOrthographic(
            properties->exists("zoomX") ? properties->getFloat("zoomX") : Toolkit::cur()->getViewport().width,
            properties->exists("zoomY") ? properties->getFloat("zoomY") : Toolkit::cur()->getViewport().height,
            aspectRatio, nearPlane, farPlane);
        break;
    }
...
{
    serializer->writeEnum("mode", "gameplay::Camera::Mode", static_cast<int>(_type), -1);
...
{
    _type = static_cast<Camera::Type>(serializer->readEnum("mode", "gameplay::Camera::Mode", -1));
    if (_type == Type::PERSPECTIVE)
    {
