 ...
 */
class Node : public Transform, public Serializable, public Ref
{
    friend class Scene;
...
public:
    ...
    */
    ...
    template<typename T>
    void addComponent(T *comp, bool dirtyBounds = true) {
        auto _comp = getComponent<T>();
        if (_comp == comp)
            return;
        ...
        if (_comp)
        {
            removeComponent<T>();
            _comp->setNode(NULL);
            Ref* ref = dynamic_cast<Ref*>(_comp);
            if (ref)
                ref->release();
        }
        ...
        if (dirtyBounds) setBoundsDirty();
    }

    template<typename T>
    T *getComponent() const {
        for (int i=0; i<_components.size(); ++i) {
            auto r = _components[i];
            T *t = dynamic_cast<T*>(r);
            if (t != NULL) return t;
        }
        return NULL;
    }
