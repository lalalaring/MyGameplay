 ...
 */
class Scene : public Serializable, public Ref
{
public:
...
{
    visitNode(_rootNode, instance, visitMethod);
}

template <class T, class C>
void Scene::visit(T* instance, bool (T::*visitMethod)(Node*,C), C cookie)
{
    visitNode(_rootNode, instance, visitMethod, cookie);
}

inline void Scene::visit(const char* visitMethod)
{
    visitNode(_rootNode, visitMethod);
}
