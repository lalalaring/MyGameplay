...
Animation::Channel::Channel()
    : _animation(NULL), _target(NULL), _propertyId(0), _curve(NULL), _duration(0)
{
...
void Animation::write(Stream* file) {
    ...
    file->writeUInt16(_channels.size());
...
Animation* Animation::read(Stream* file) {
    std::string id = file->readStr();
    Animation* a = new Animation(id.c_str());
