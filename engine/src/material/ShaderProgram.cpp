...
ShaderProgram* ShaderProgram::createFromFile(const char* vshPath, const char* fshPath, const char* defines)
{
    ...
    ShaderProgram* effect = createFromSource(vshPath, vshSource, fshPath, fshSource, defines);
...
ShaderProgram* ShaderProgram::createFromSource(const char* vshPath, const char* vshSource, const char* fshPath, const char* fshSource, const char* defines)
{
    GP_ASSERT(vshSource);
    GP_ASSERT(fshSource);
...
Uniform* ShaderProgram::getUniform(const char* name) const
{
    ...
    if (len>3 && name[len-1] == ']' && name[len - 2] == '0' && name[len - 3] == '[') {
