...
_type(MaterialParameter::NONE), _count(1), _dynamic(false), _name(name ? name : ""), _uniform(NULL), _loggerDirtyBits(0), _methodBinding(NULL), _temporary(false)
{
    clearValue();
}
...
{
    if (enumName.compare("gameplay::MaterialParameter::Type") == 0)
    {
        switch (value)
        {
...
void MaterialParameter::onSerialize(Serializer* serializer) {
    ...
    serializer->writeEnum("type", "gameplay::MaterialParameter::Type", static_cast<int>(_type), -1);
...
void MaterialParameter::onDeserialize(Serializer* serializer) {
    ...
    _type = static_cast<MaterialParameter::Type>(serializer->readEnum("minFilter", "gameplay::MaterialParameter::Type", -1));
