...
gameplay::Ptr<Project> EditorWindow::getProject() const
{
...
gameplay::Ptr<gameplay::Scene> EditorWindow::getScene() const
{
    return _scene;
}
...
void EditorWindow::onOpenScene(const QString& path)
{
    if (path.compare(_scenePath) != 0)
    {
        ...
        _scene = dynamic_cast<gameplay::Scene*>(reader->readObject(nullptr));
