    ...
    _slopeAngle(0.0f), _cosSlopeAngle(1.0f), _physicsEnabled(true), _mass(mass), _actionInterface(NULL)
{
    ...
    PhysicsController::cur()->_world->addAction(_actionInterface);
}
...
PhysicsCharacter::~PhysicsCharacter()
{
    ...
    GP_ASSERT(PhysicsController::cur() && PhysicsController::cur()->_world);
    PhysicsController::cur()->_world->removeAction(_actionInterface);
...
void PhysicsCharacter::jump(float height, bool force)
{
    ...
    Vector3 jumpVelocity = PhysicsController::cur()->getGravity() * height * 2.0f;
    jumpVelocity.set(
...
void PhysicsCharacter::stepForwardAndStrafe(btCollisionWorld* collisionWorld, float time)
{
    ...
    while (fraction > btScalar(0.01) && maxIter-- > 0)
    {
        ...
        if (callback.hasHit())
        {
            ...
            PhysicsCollisionObject* o = PhysicsController::cur()->getCollisionObject(callback.m_hitCollisionObject);
            GP_ASSERT(o);
...
void PhysicsCharacter::stepDown(btCollisionWorld* collisionWorld, btScalar time)
{
    ...
    btVector3 gravity = PhysicsController::cur()->_world->getGravity();
...
bool PhysicsCharacter::fixCollision(btCollisionWorld* world)
{
    ...
    for (int i = 0, count = pairCache->getNumOverlappingPairs(); i < count; ++i)
    {
        ...
        for (int j = 0, manifoldCount = _manifoldArray.size(); j < manifoldCount; ++j)
        {
            ...
            // Skip ghost objects.
            PhysicsCollisionObject* object = PhysicsController::cur()->getCollisionObject(
