...
PhysicsCollisionObject* PhysicsCollisionObject::load(const std::string& url, Node* node) {
    ...
    if (const char* type = properties->getString("type"))
    {
        ...
        {
            _collisionObject = PhysicsCharacter::create(node, properties);
        }
        else if (strcmp(type, "GHOST_OBJECT") == 0)
        {
            _collisionObject = PhysicsGhostObject::create(node, properties);
        }
        else if (strcmp(type, "RIGID_BODY") == 0)
        {
            _collisionObject = PhysicsRigidBody::create(node, properties);
        }
        else if (strcmp(type, "VEHICLE") == 0)
        {
            _collisionObject = PhysicsVehicle::create(node, properties);
        }
        else if (strcmp(type, "VEHICLE_WHEEL") == 0)
        {
            ...
            //
            _collisionObject = PhysicsVehicleWheel::create(node, properties);
        }
        else
        {
    ...
    node->addComponent<PhysicsCollisionObject>(_collisionObject);
    return _collisionObject;
}

PhysicsCollisionObject* PhysicsCollisionObject::create(Node* node, PhysicsCollisionObject::Type type, const PhysicsCollisionShape::Definition& shape,
