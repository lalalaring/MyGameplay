...
PhysicsCollisionShape* PhysicsController::createMesh(Mesh* mesh, const Vector3& scale, bool dynamic)
{
    ...
    char* vertexData = (char*)mesh->_vertexData;
    if (vertexData == NULL)
    {
    ...
    Vector3 v;
    int vertexStride = mesh->getVertexSize();
    for (unsigned int i = 0; i < vertexCount; i++)
    {
        v.set(*((float*)&vertexData[i * vertexStride + 0 * sizeof(float)]),
                *((float*)&vertexData[i * vertexStride + 1 * sizeof(float)]),
                *((float*)&vertexData[i * vertexStride + 2 * sizeof(float)]));
        v *= m;
    ...
    if (dynamic)
    {
        ...
        btConvexHullShape* originalConvexShape = bullet_new<btConvexHullShape>(shapeMeshData->vertexData, vertexCount, sizeof(float)*3);
    ...
    else
    {
        ...
        if (partCount > 0)
        {
            ...
            for (size_t i = 0; i < partCount; i++)
            {
                ...
                shapeMeshData->indexData.push_back((unsigned char*)meshPart->_indexData);
                meshPart->_indexData = NULL;
        ...
        else
        {
            ...
            indexedMesh.m_numTriangles = vertexCount / 3; // assume TRIANGLES primitive type
...
bool PhysicsController::checkConstraintRigidBodies(PhysicsRigidBody* a, PhysicsRigidBody* b)
{
    ...
    {
        GP_ASSERT(a->_node);
        GP_ERROR("Rigid body '%s' does not support constraints; unexpected behavior may occur.", a->_node->getName());
        return false;
    }
    ...
    {
        GP_ASSERT(b->_node);
        GP_ERROR("Rigid body '%s' does not support constraints; unexpected behavior may occur.", b->_node->getName());
        return false;
    }
       ...
       btIDebugDraw::DBG_DrawContactPoints | btIDebugDraw::DBG_DrawWireframe), _meshBatch(NULL), _lineCount(0)
{
    ...
    ShaderProgram* effect = ShaderProgram::createFromSource(vs_str, fs_str);
...
PhysicsController::Listener::~Listener()
{
    ...
    PhysicsController::cur()->removeStatusListener(this);
}
