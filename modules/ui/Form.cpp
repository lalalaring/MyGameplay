...
void Form::finishBatch(BatchableLayer* batch)
{
    if (!_batched)
    {
        batch->finish();
...
unsigned int Form::draw(RenderView* view)
{
    ...
    else
    {
        ...
        Renderer::cur()->clear(Renderer::CLEAR_DEPTH, 0,0,0,0, 1, 0);

        // Setup an ortho matrix that maps to the current viewport
        const Rectangle& viewport = Toolkit::cur()->getViewport();
...
bool Form::pointerEventInternal(bool mouse, int evt, int x, int y, int param)
{
    ...
    if (mouse && Toolkit::cur()->isMouseCaptured())
        return false;
...
bool Form::projectPoint(int x, int y, Vector3* point)
{
    ...
    if (scene && (camera = scene->getActiveCamera()))
    {
        ...
        Ray ray;
        camera->pickRay(Toolkit::cur()->getViewport(), x, y, &ray);
