...
unsigned int TextBox::drawText(Form* form, const Rectangle& clip)
{
    ...
    if (_font)
    {
        ...
        //_font->start();
        _font->drawText(displayedText.c_str(), _textBounds.x, _textBounds.y, _textColor, fontSize);
        //_font->finish();
        finishBatch(form, _font);
...
void TextBox::setCaretLocation(int x, int y)
{
    ...
    Font* font = getFont(state);
    unsigned int fontSize = getFontSize(state);
    Text::Justify textAlignment = getTextAlignment(state);
    bool rightToLeft = getTextRightToLeft(state);
    const std::string displayedText = getDisplayedText();

    int index = -1;// font->getIndexAtLocation(displayedText.c_str(), _textBounds, fontSize, point, &point,
    ...
    /*
    {
        ...
        unsigned int w;
        unsigned int h;
        font->measureText(displayedText.c_str(), fontSize, &w, &h);
