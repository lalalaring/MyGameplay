...
void Control::setTextAlignment(Text::Justify alignment, unsigned char states)
{
    overrideStyle();
    Theme::Style::Overlay* overlays[Theme::Style::OVERLAY_MAX] = { 0 };
    getOverlays(states, overlays);
...
Text::Justify Control::getTextAlignment(State state) const
{
    Theme::Style::Overlay* overlay = getOverlay(state);
    GP_ASSERT(overlay);
...
void Control::finishBatch(Form* form, BatchableLayer* batch)
{
...
void Control::overrideThemedProperties(Properties* properties, unsigned char states)
{
    ...
    {
        setTextAlignment(Text::getJustify(properties->getString("textAlignment")), states);
    }
