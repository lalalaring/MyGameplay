...
std::vector<GLFrameBuffer*> GLFrameBuffer::_frameBuffers;
...
GLFrameBuffer::~GLFrameBuffer()
{
    ...
    std::vector<GLFrameBuffer*>::iterator it = std::find(_frameBuffers.begin(), _frameBuffers.end(), this);
...
FrameBuffer* GLFrameBuffer::create(const char* id)
{
    return create(id, 0, 0);
}

FrameBuffer* GLFrameBuffer::create(const char* id, unsigned int width, unsigned int height, Texture::Format format)
{
...
FrameBuffer* GLFrameBuffer::getFrameBuffer(const char* id)
{
    GP_ASSERT(id);

    // Search the vector for a matching ID.
    std::vector<GLFrameBuffer*>::const_iterator it;
...
void GLFrameBuffer::setRenderTarget(RenderTarget* target, unsigned int index)
{
...
void GLFrameBuffer::setRenderTarget(RenderTarget* target, Texture::CubeFace face, unsigned int index)
{
...
void GLFrameBuffer::setRenderTarget(RenderTarget* target, unsigned int index, GLenum textureTarget)
{
...
RenderTarget* GLFrameBuffer::getRenderTarget(unsigned int index) const
{
    GP_ASSERT(_renderTargets);
...
void GLFrameBuffer::createDepthStencilTarget(int format) {
    DepthStencilTarget *dst = DepthStencilTarget::create(this->_id.c_str(), format == 0 ? DepthStencilTarget::DEPTH : DepthStencilTarget::DEPTH_STENCIL, getWidth(), getHeight());
    setDepthStencilTarget(dst);
    dst->release();
}
...
FrameBuffer* GLFrameBuffer::bind(GLenum type)
{
...
Image* GLFrameBuffer::createScreenshot(Image::Format format)
{
