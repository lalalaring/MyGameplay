...
ScriptController* ScriptController::cur()
{
    return g_cur;
}
...
void ScriptController::initialize()
{
    ...
    int argc;
    char** argv;
    Toolkit::cur()->getArguments(&argc, &argv);
    args << "arg = { }\n";
...
void ScriptController::schedule(float timeOffset, const char* function)
{
    ...
    Toolkit::cur()->schedule(timeOffset, listener, NULL);
}
...
void ScriptController::ScriptTimeListener::timeEvent(long timeDiff, void* cookie)
{
    ...
    std::list<ScriptTimeListener*>& list = ScriptController::cur()->_timeListeners;
    std::list<ScriptTimeListener*>::iterator itr = std::find(list.begin(), list.end(), this);
    if (itr != list.end())
        list.erase(itr);

    // Call the script function
    ScriptController::cur()->executeFunction<void>(script, function.c_str(), "l", NULL, timeDiff);
...
{
    lua_pushcfunction(ScriptController::cur()->_lua, cppFunction);
    lua_setglobal(ScriptController::cur()->_lua, luaFunction);
}
...
{
    if (lua_type(ScriptController::cur()->_lua, index) == LUA_TSTRING)
        return luaL_checkstring(ScriptController::cur()->_lua, index);
    else if (lua_type(ScriptController::cur()->_lua, index) == LUA_TNIL && !isStdString)
        return NULL;
    else
    {
