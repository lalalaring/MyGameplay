...
class MainApp : public Game {
    ...
    void initialize() {
        ...
        SAFE_RELEASE(mesh);
        Material* material = _model->setMaterial("res/shaders/enviro.vert", "res/shaders/water.frag");
        material->getParameter("u_albedo")->setVector3(Vector3(0.0, 0.1176, 0.058));
        //material->getParameter("u_roughness")->setFloat(0.1);
        //material->getParameter("u_metallic")->setFloat(0.1);
        material->getParameter("u_skybox")->setSampler(cube->getTexture());
        material->getParameter("u_waterNormal")->setSampler("res/image/waternormals.jpg", true);

        Node* modelNode2 = _scene->addNode("model");
        modelNode2->setDrawable(_model);
        modelNode2->scale(10000);
        modelNode2->setTranslationY(-100);
    }
